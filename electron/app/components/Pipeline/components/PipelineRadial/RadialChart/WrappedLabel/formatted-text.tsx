...
const FormattedText = ({ currentFontSize, display, name, containerWidth, containerHeight }: Props): RadialText => {
  let nodeName = name;
  ...
  const shouldLineBreak = containerHeight > 30 || display === "inline";
  ...
  if (!shouldLineBreak) {
    ...
    for (let i = 0; i < words.length; i += 1) {
      ...
      } else if (combinedWidth < containerWidth) {
        span = [...span, tSpans[i]];
        lines = [...lines, span];
        maxWidth = Math.max(maxWidth, combinedWidth);
      } else {
  ...
  const height = fontSize * lines.length * mod;
  if (height > containerHeight) {
    fontSize *= containerHeight / height;
  }
